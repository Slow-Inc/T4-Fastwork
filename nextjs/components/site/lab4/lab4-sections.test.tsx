...
import { Lab4SolutionSelector, type Lab4Solution } from './lab4-solution-selector';
import { Lab4Schematic, type Lab4StackNode, type Lab4ProcessStep } from './lab4-schematic';
import { Lab4Services, type Lab4Service } from './lab4-services';

afterEach(cleanup);

describe('Lab4SolutionSelector', () => {
  ...
  test('renders one linked row per solution pointing at /recommend/[type]', () => {
    const { container } = render(<Lab4SolutionSelector items={solutions} />);
    expect(container.querySelectorAll('a.lab4-sol').length).toBe(6);
  ...
  test('varies composition via the span modifier (no 6 identical cards)', () => {
    const { container } = render(<Lab4SolutionSelector items={solutions} />);
    expect(container.querySelectorAll('.lab4-sol.wide').length).toBe(3);
    expect(container.querySelectorAll('.lab4-sol.narrow').length).toBe(3);
  });
});

describe('Lab4Schematic', () => {
  const stack: Lab4StackNode[] = [
    ...
    { id: 'ai', label: 'AI', tech: 'LLM · RAG', desc: 'ตอบจากผลงานจริง', href: '/chat' },
  ];
  ...
  test('renders the five real stack nodes with their tech labels', () => {
    const { container } = render(<Lab4Schematic stack={stack} process={process} />);
    expect(container.querySelectorAll('.lab4-node').length).toBe(5);
    expect(screen.getByText('Supabase · pgvector')).toBeDefined();
  });
  test('the AI node is a real link into /chat (§14.18 — schematic opens the live chatbot)', () => {
    render(<Lab4Schematic stack={stack} process={process} />);
    const ai = screen.getByRole('link', { name: /AI/ });
    expect(ai.getAttribute('href')).toBe('/chat');
  });
  test('renders the five process steps in order', () => {
    const { container } = render(<Lab4Schematic stack={stack} process={process} />);
    const steps = Array.from(container.querySelectorAll('.lab4-step .t')).map(
      (el) => el.textContent,
    );
...
describe('Lab4Services', () => {
  ...
  test('renders a rung per service with a complexity meter scaled by level', () => {
    const { container } = render(<Lab4Services items={services} />);
    expect(container.querySelectorAll('.lab4-svc').length).toBe(3);
    const meters = container.querySelectorAll<HTMLElement>('.lab4-svc .meter i');
