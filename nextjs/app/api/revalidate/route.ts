 ...
 * the allowlist covers project and content public paths only.
 */
...
export async function POST(request: Request): Promise<Response> {
  ...
  const targets = kind && ['faq', 'service', 'certificate', 'blog'].includes(kind)
    ? contentRevalidationTargets(kind)
    : revalidationTargets(slug);
