...
- 🚧 **HARD GATE at C2 #53 — GitHub OAuth needs a human-registered OAuth app** (client id/secret in a dashboard). Everything downstream (C3 member edit UI, C4 additive, C6 admin) depends on member auth → autonomous work is blocked here until the OAuth app + secrets exist. C5 (read-path /team → DB) is agent-doable but low marginal value (pattern already proven by the carousel).
