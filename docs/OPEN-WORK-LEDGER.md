...
- 🟡 **C2 #53 flow** shipped (`ce4bc5e`) — `/member/login` → `signInWithOAuth(github)` → `/auth/callback` (exchange + `link_current_member()` RPC) → `/member` gated by `getCurrentMember()`; members RLS applied to prod. **`/security-review` found + fixed 2 issues**: HIGH account-takeover (link keyed off spoofable `user_metadata` → now reads GoTrue-owned `auth.identities`) + LOW column exposure (anon column-scoped grant). 247 unit + 52 e2e green; signed-out redirect verified. **⏳ ONLY remaining: a human logs in at `/member/login` with a team GitHub account to confirm the round-trip lands on `/member` as that member.** Then C2 done → unblocks C3/C4/C6.
